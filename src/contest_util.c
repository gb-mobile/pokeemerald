...
void LoadLinkContestPlayerPalettes(void)
{
    ...
    if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)
    {
        for (i = 0; i < gNumLinkContestPlayers; i++)
        {
            ...
            {
                if (gLinkPlayers[i].gender == MALE)
                    sprite->oam.paletteNum = LoadObjectEventPalette(OBJ_EVENT_PAL_TAG_RS_BRENDAN, FALSE);
                else
                    sprite->oam.paletteNum = LoadObjectEventPalette(OBJ_EVENT_PAL_TAG_RS_MAY, FALSE);
            }
            else
            {
                if (gLinkPlayers[i].gender == MALE)
                    sprite->oam.paletteNum = LoadObjectEventPalette(OBJ_EVENT_PAL_TAG_BRENDAN, FALSE);
                else
                    sprite->oam.paletteNum = LoadObjectEventPalette(OBJ_EVENT_PAL_TAG_MAY, FALSE);
            }
        }
    }
}
...
void ShowContestEntryMonPic(void)
{
    ...
    if (FindTaskIdByFunc(Task_ShowContestEntryMonPic) == TASK_NONE)
    {
        ...
        HandleLoadSpecialPokePic(TRUE, gMonSpritesGfxPtr->spritesGfx[B_POSITION_OPPONENT_LEFT], species, personality, VERSION_EMERALD);
